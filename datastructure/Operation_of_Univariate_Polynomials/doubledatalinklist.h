...
#ifndef DOUBLEDATALINKLIST_H
#define DOUBLEDATALINKLIST_H
using namespace std;
...
public:
    ...
    bool Insert(int position,const ElemType& e,const ElemType& p);
    void Clear();//清空整个链表
};
...
*/
template<class ElemType>
bool Linklist<ElemType>::Insert(int position,const ElemType& e,const ElemType& p) {
    if (position<1||position>Length()+1) {
        return false;
    }
    temPtr=(position==1)?head:GetElemPtr(position-1);
    Node<ElemType>* newPtr=new Node<ElemType>(temPtr->next,e,p);
    temPtr->next=newPtr;
    return true;
}//在第position个位置插入一个节点
